  ...
  renderTable(users, handlers) {
    ...
    users.forEach((u, i) => {
      ...
      row.innerHTML = `
        ...
        <td data-label="Username"><div class="um-username-cell um-username-clickable" tabindex="0" role="button" title=${esc(u.username)}>${esc(u.username)}${u.authUserId ? ' <span class="pill pill-linked" title="Supabase Auth account — linked">Linked</span>' : ' <span class="pill pill-bad" title="Supabase Auth account — not linked">Unlinked</span>' }</div></td>
